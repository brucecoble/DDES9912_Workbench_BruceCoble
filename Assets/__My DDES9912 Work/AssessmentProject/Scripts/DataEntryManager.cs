...
public class DataEntryManager : MonoBehaviour
{
    public float moveSpeed = 2f;  // Movement speed
    public InteractableGeneral subject; // Assign the object with the Interactable for generating the interaction
    //private GameObject targetGameObject; // Assign your target GameObject in the Inspector
    ...
    public float subtractXAmount = 0.00f; // The value to subtract from the X position
    public float subtractYAmount = -0.14f; // The value to subtract from the X position
    ...
    IEnumerator ProcessData()
    {
        ...
        foreach (GameObject button in buttonsToPress)
        {
            if (button != null)
            {
                ...
                // Move the pusher to the button
                //StartCoroutine(MoveToXZY(button));


                // Get target position
                Vector3 targetPos = button.transform.position;
    ...
    IEnumerator MoveToXZY(GameObject targetGameObject)
    {
        // Get target position
